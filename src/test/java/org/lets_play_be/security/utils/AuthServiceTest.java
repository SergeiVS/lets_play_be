...
class AuthServiceTest {
    ...
    void login_NotAuthenticated() {
        ...
        assertThrows(RestException.class, () -> authService.login(loginRequest, httpResponse), "User is not authenticated");
    ...
    void refreshAccessToken_Success() {
        ...
        when(jwtService.generateAccessTokenCookie(user.getEmail(), profile.roles()))
                .thenReturn(ResponseCookie.from("access-token", "new-access-token")
                        .path("/")
                        .maxAge(5000)
                        .httpOnly(true)
                        .build());
        ...
        assertThat(httpResponse.getCookie("access-token")).isNull();

        LoginResponse result = authService.refreshAccessToken(httpRequest, httpResponse);

        assertThat(Objects.requireNonNull(httpResponse.getCookie("access-token")).getValue())
    ...
    @Test
    void refreshAccessToken_ThrowsRestException_TokenNotValid() {
        String refreshToken = "refresh-token";
        httpRequest.setCookies(new Cookie("access-token", "access-token")
                , new Cookie("refresh-token", "refresh-token"));
        ...
        assertThrows(RestException.class, () -> authService.refreshAccessToken(httpRequest, httpResponse), "Refresh token is not valid");
